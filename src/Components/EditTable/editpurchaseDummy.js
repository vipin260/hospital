...
const PurchaseAdd = (props) => {
  ...
  const [purchase, setpurchase] = useState({
    // id                : "",
    supplier_name     : SinglePurchaseData.data.supplier_name,
    date_of_purchase  : SinglePurchaseData.data.date_of_purchase,
    invoice_no        : SinglePurchaseData.data.invoice_no,
  });
                  ...
                  <TableContainer>
                    <Table sx={{marginBottom:2}}>
                      ...
                      { inputAdd.map((singleService, index) => {
                           const ValueName =()=>{
                            let arr = FetchProductData.filter((items) => singleService.item_name === items.value) 
                            for(let i=0;i<arr.length;i++){
                              return (arr[i].label)
                           }
                           //return (singleService.item_name)
                          }
                          //console.log('Name is', ValueName())
                        return(
                     <TableBody  key={index} >
                      <TableRow >
                        <TableCell name='moreinput' align="right">{singleService.item_name}</TableCell>
