...
import { FetchProduct, FetchSingleProduct, DeleteProduct, FetchProductOpd } from "../../redux/action/Actions";
...
const OpdFetch = () => {
  ...
  const FetchProductData = useSelector((state) => state.ProductReducersData.OpdFetch);
 

  console.log(' datas of opd',FetchProductData);
  ...
  useEffect(() => {
    let data = {"action" : "getProductOPD"}
    dispatch(FetchSingleProduct())
    dispatch(FetchProductOpd(data))
 }, [dispatch])
