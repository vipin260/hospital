...
import { FetchProduct, FetchSingleProduct, DeleteProduct, FetchProductOptical } from "../../redux/action/Actions";
...
const OpticalFetch = () => {
  ...
  const FetchProductData = useSelector((state) => state.ProductReducersData.OpticalFetch);
 

  console.log(' data of optical are',FetchProductData);
  ...
  useEffect(() => {
    let data = {"action" : "getProductOptical"}
    dispatch(FetchSingleProduct())
    dispatch(FetchProductOptical(data))
 }, [dispatch])
